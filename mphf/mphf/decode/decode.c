...
int main (int argc, char** argv) {
  ...
  if (argc > 1) {
    ...
    printf ("c use as follows: ./decode #ELEMENTS SEED SOLUTION\n"); exit (0); }
  ...
  if (argc > 3) {
    solution = fopen (argv[3], "r");
    fseek (solution, 0, SEEK_END);
    m = ftell (solution);
    fseek (solution, 0, SEEK_SET); }
  else {
    solution = stdin;
    m = 1000;
  ...
  m = i - 1;
  if (m == -1) { printf ("c no solution file (formula UNSAT?)\n"); exit (0); }
